...
void CVehicle::InjectHooks() {
    ...
    RH_ScopedOverloadedInstall(IsPassenger, "Ped", 0x6D1BD0, bool(CVehicle::*)(const CPed*) const);
...
// 0x6E2D60
void* CVehicle::operator new(size_t size, int32 poolRef) {
...
// 0x6D6640
void CVehicle::SpecialEntityPreCollisionStuff(CPhysical* physical, bool doingShift, bool& skipTestEntirely, bool& skipCol, bool& forceBuildingCol, bool& forceSoftCol) {
    if (physical->GetIsTypePed()
        && physical->AsPed()->bKnockedOffBike
        && physical->AsPed()->m_pVehicle == this) {
        skipTestEntirely = true;
        return;
    }

    if (physicalFlags.bSubmergedInWater
        && GetStatus() != STATUS_PLAYER
        && (GetStatus() != STATUS_REMOTE_CONTROLLED && physical->DoesNotCollideWithFlyers())) // BUG:? Seems like it should check for it being heli
    {
        skipTestEntirely = true;
        return;
    }

    if (m_pEntityIgnoredCollision == physical || physical->m_pEntityIgnoredCollision == this) {
        skipCol = true;
        physicalFlags.bSkipLineCol = true;
        return;
    }

    if (m_pAttachedTo == physical) {
        skipCol = true;
        return;
    }

    if (physical->m_pAttachedTo == this) {
        skipTestEntirely = true;
        physicalFlags.bSkipLineCol = true;
        return;
    }

    if (physicalFlags.bDisableCollisionForce && physical->physicalFlags.bDisableCollisionForce) {
        skipTestEntirely = true;
        return;
    }
    ...
    if (GetIsStuck()
        ...
        && (physical->AsVehicle()->physicalFlags.bDisableCollisionForce
            && !physical->AsVehicle()->physicalFlags.bCollidable)) {
        skipCol = true;
        physicalFlags.bSkipLineCol = true;
        return;
    }

    if (physical->IsImmovable()) {
        if (doingShift) {
            skipCol = true;
        } else if (GetIsStuck() || physical->GetIsStuck()) {
            forceSoftCol = true;
        }
    ...
    if (physical->GetIsTypeObject()) {
        if (physical->AsObject()->IsFallenLampPost()) {
            skipTestEntirely = true;
            physical->AsObject()->m_pEntityIgnoredCollision = this;
        } else {
            if (physical->IsModelTempCollision()) {
                skipTestEntirely = true;
                return;
            }

            if (physical->AsObject()->IsTemporary()
                || physical->AsObject()->IsExploded()
                || !physical->GetIsStatic()) {
                if (IsConstructionVehicle()) {
                    if (GetIsStuck() || physical->GetIsStuck()) {
                        forceSoftCol = true;
                    }
                } else if (!physical->AsObject()->CanBeSmashed() && !IsBike()) {
                    auto  tempMat = CMatrix();
                    auto* cm = physical->GetColModel();
                    ...
                    if (vecMax.x < 1.0F && vecMax.y < 1.0F && vecMax.z < 1.0F) {
                        ...
                        const auto vecTransformed = physical->m_matrix->TransformPoint(vecSize);
    ...
    if (IsRCCar() && (physical->GetIsTypeVehicle() || physical->GetIsTypePed())) {
        skipCol = true;
        physicalFlags.bSkipLineCol = true;
        return;
    }

    if (physical == m_pTowingVehicle || physical == m_pVehicleBeingTowed) {
        forceSoftCol = true;
        physicalFlags.bSkipLineCol = true;
        return;
    }

    if (physical->GetIsStuck()) {
        forceBuildingCol = true;
        return;
    }
}
...
// 0x6D0E90
uint8 CVehicle::SpecialEntityCalcCollisionSteps(bool& doPreCheckAtFullSpeed, bool& doPreCheckAtHalfSpeed) {
    ...
    if (fLongestDir < 1.0F) {
        doPreCheckAtFullSpeed = true;
    } else if (fLongestDir < 2.0F) {
        doPreCheckAtHalfSpeed = true;
    }
...
void CVehicle::PreRender() {
    ...
    float envMapCoeff = CPhysical::GetLightingFromCol(false);
    GetVehicleModelInfo()->SetEnvMapCoeff(envMapCoeff * 0.4F);
}

// Modificated in Mobile
// 0x6D0E60
void CVehicle::Render() {
    CVehicleModelInfo::SetDirtTextures(GetVehicleModelInfo(), GetBodyDirtLevelUInt8());
    CEntity::Render();
}
...
// 0x6D56C0
void CVehicle::ProcessOpenDoor(CPed* ped, uint32 doorComponentId_, uint32 animGroupId, uint32 animId, float currTime) {
    ...
    eDoors iCheckedDoor = [&] {
        ...
        default:
            ...
            return static_cast<eDoors>(currTime);
        }
    }();
    ...
    case ANIM_ID_CAR_OPEN_RHS_1: {
        ...
        if (currTime < fAnimStart) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, 0.0F, false);
        } else if (currTime > fAnimEnd) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, 1.0F, true);
        } else if (currTime > fAnimStart && currTime < fAnimEnd) {
            const auto fNewRatio = invLerp(fAnimStart, fAnimEnd, currTime);
    ...
    case ANIM_ID_CAR_CLOSE_RHS_1: {
        ...
        if (currTime < fAnimStart) {
            ...
            const auto fNewRatio = 1.0F - invLerp(fAnimStart, fAnimEnd, currTime);
            const auto fCurRatio = GetDooorAngleOpenRatio(iCheckedDoor);
            if (fCurRatio < fNewRatio) {
                OpenDoor(ped, doorComponentId, iCheckedDoor, fNewRatio, true);
            }
    ...
    case ANIM_ID_CAR_JACKEDRHS:
        if (currTime < 0.1F) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, 0.0F, true);
        } else if (currTime > 0.4F) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, 1.0F, true);
        } else if (currTime > 0.1F && currTime < 0.4F) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, (currTime - 0.1F) * (10.0F / 3.0F), true);
        }
        return;
    ...
    case ANIM_ID_CAR_ROLLDOOR: {
        ...
        if (currTime > fAnimEnd2) {
            OpenDoor(ped, doorComponentId, iCheckedDoor, 0.0F, true);
        } else if (currTime > fAnimStart && currTime < fAnimEnd) {
            const auto fNewRatio = invLerp(fAnimStart, 0.2F, currTime);
            const auto fCurRatio = GetDooorAngleOpenRatio(iCheckedDoor);
            if (fCurRatio < fNewRatio) {
                OpenDoor(ped, doorComponentId, iCheckedDoor, fNewRatio, true);
            }
        } else if (currTime > fAnimEnd && currTime < fAnimEnd2) {
            const auto fNewRatio = 1.0F - invLerp(fAnimEnd, fAnimEnd2, currTime);
            const auto fCurRatio = GetDooorAngleOpenRatio(iCheckedDoor);
            if (fCurRatio > fNewRatio) {
                OpenDoor(ped, doorComponentId, iCheckedDoor, fNewRatio, true);
            }
...
void CVehicle::ProcessDrivingAnims(CPed* driver, bool blend) {
    ...
    // 0x6DF6DA
    uint32 driveByAnimIds[] = { ANIM_ID_DRIVEBY_L, ANIM_ID_DRIVEBY_R, ANIM_ID_DRIVEBYL_L, ANIM_ID_DRIVEBYL_R };
    CAnimBlendAssociation* driveByAnim = nullptr;
    for (const auto id : driveByAnimIds) {
        driveByAnim = RpAnimBlendClumpGetAssociation(driver->m_pRwClump, id);
        if (driveByAnim) {
            break;
        }
    }

    if (!vehicleFlags.bLowVehicle && m_GasPedal < 0.0F && !driveByAnim
    ...
    if (m_fSteerAngle == 0.0F || driveByAnim) {
        if (lookLeftAnim) {
            lookLeftAnim->m_BlendDelta = -4.0F;
        }
        if (lookRightAnim) {
            lookRightAnim->m_BlendDelta = -4.0F;
        }
        if (lookBackAnim) {
...
// 0x6D1F30
bool CVehicle::CanPedStepOutCar(bool ignoreSpeedCheck) const {
    const auto fUpZ = m_matrix->GetUp().z;
    ...
    return m_vecMoveSpeed.Magnitude2D() <= 0.01F && std::fabs(m_vecMoveSpeed.z) <= 0.05F && m_vecTurnSpeed.SquaredMagnitude() <= 0.0004F;
}
...
bool CVehicle::CanPedJumpOutCar(CPed* ped) {
    ...
    if (!IsSubPlane()
        && !IsSubHeli()
        && (!IsAutomobile() || m_matrix->GetUp().z >= 0.3F || m_nLastCollisionTime <= CTimer::GetTimeInMS() - 1'000)) {
    ...
    const auto fMoveSpeedSquared = m_vecMoveSpeed.SquaredMagnitude();
...
// 0x6DFB70
bool CVehicle::GetTowHitchPos(CVector& result, bool genericPosOk, CVehicle* vehicle) {
    if (!genericPosOk) {
        return false;
    }

    const auto fColFront = CModelInfo::GetModelInfo(m_nModelIndex)->GetColModel()->GetBoundingBox().m_vecMax.y;
    result.Set(0.0F, fColFront + 1.0F, 0.0F);
    result = m_matrix->TransformPoint(result);
    return true;
}

// 0x6DFBE0
bool CVehicle::GetTowBarPos(CVector& result, bool genericPosOk, CVehicle* vehicle) {
    if (!genericPosOk) {
        return false;
    }

    const auto fColRear = CModelInfo::GetModelInfo(m_nModelIndex)->GetColModel()->GetBoundingBox().m_vecMin.y;
    result.Set(0.0F, fColRear - 1.0F, 0.0F);
    result = m_matrix->TransformPoint(result);
    return true;
}
...
// 0x6D0C00
void CVehicle::SetRemap(int32 index) {
    if (index == -1) {
        SetRemapTexDictionary(-1);
    } else {
        const auto infoRemapInd = GetVehicleModelInfo()->m_anRemapTxds[index];
...
eVehicleAppearance CVehicle::GetVehicleAppearance() const {
    uint32 flags = (m_pHandlingData->m_nModelFlags & (VEHICLE_HANDLING_MODEL_IS_BOAT | VEHICLE_HANDLING_MODEL_IS_PLANE | VEHICLE_HANDLING_MODEL_IS_HELI | VEHICLE_HANDLING_MODEL_IS_BIKE));
...
// 0x6D10E0
bool CVehicle::CustomCarPlate_TextureCreate(CVehicleModelInfo* mi) {
    m_CustomPlateTexture = nullptr;
    ...
    if (const auto text = mi->GetCustomCarPlateText()) {
        m_CustomPlateTexture = CCustomCarPlateMgr::CreatePlateTexture(text, mi->m_nPlateType);
        mi->SetCustomCarPlateText(text);
        mi->m_nPlateType = -1;
    } else {
        m_CustomPlateTexture = RpMaterialGetTexture(mi->m_pPlateMaterial);
...
// 0x6D1180
bool CVehicle::CanBeDeleted() const {
...
// 0x6D1230
float CVehicle::ProcessWheelRotation(tWheelState wheelState, const CVector& forward, const CVector& wheelSpeed, float radius) {
    ...
    const auto angle = DotProduct(forward, wheelSpeed) / radius;
    return -angle;
}

// 0x6D1280
bool CVehicle::CanVehicleBeDamaged(CEntity* damager, eWeaponType weaponUsed, bool* beingShotAt) {
    ...
    *beingShotAt = false;
    if (CanPhysicalBeDamaged(weaponUsed, beingShotAt)) {
        return !beingShotAt || GetStatus() != STATUS_PLAYER || m_fHealth >= 250.0f;
    } else {
        return false;
    }
}
...
// 0x6D1BD0
bool CVehicle::IsPassenger(const CPed* ped) const {
    if (!ped) {
        return false;
    }
...
// 0x6D1E80
bool CVehicle::CanPedEnterCar() const {
...
// 0x6D22F0
bool CVehicle::IsVehicleNormal() const {
    if (m_pDriver
        && !m_nNumPassengers
...
// 0x6D2330
void CVehicle::ChangeLawEnforcerState(bool newState) {
    if (newState) {
...
void CVehicle::ActivateBomb() {
    ...
    CGarages::TriggerMessage("GA_12", -1, 3'000u); // "Bomb armed"
}
...
// 0x6D25D0
bool CVehicle::CarHasRoof() const {
...
void CVehicle::UpdatePassengerList() {
    ...
    if (m_nNumPassengers) {
        if (rng::all_of(m_apPassengers, [](auto&& p) {
                return p == nullptr;
            })) {
            m_nNumPassengers = 0;
        }
...
// 0x6D2BF0
void CVehicle::MakeDirty(CColPoint& wheelColPoint) {
    if (g_surfaceInfos.IsWater(wheelColPoint.m_nSurfaceTypeB) || CWeather::IsRainy()) {
    ...
    if (g_surfaceInfos.MakesCarDirty(wheelColPoint.m_nSurfaceTypeB)) {
    ...
    if (g_surfaceInfos.MakesCarClean(wheelColPoint.m_nSurfaceTypeB)) {
...
// 0x6D2D50
bool CVehicle::AddWheelDirtAndWater(CColPoint& wheelColPoint, bool produceWheelDrops, bool wheelsSpinning, bool wheelInWater) {
    if (!produceWheelDrops && !g_surfaceInfos.IsSand(wheelColPoint.m_nPieceTypeB)) {
        return false;
    }

    if (wheelInWater) {
        g_fx.AddWheelSpray(this, wheelColPoint.m_vecPoint, wheelsSpinning, true, m_fContactSurfaceBrightness);
        return false;
    }

    const auto CreateFxForSurface = [&](auto CheckSurface, auto AddFx) {
        if ((g_surfaceInfos.*CheckSurface)(wheelColPoint.m_nSurfaceTypeB)) {
            (g_fx.*AddFx)(this, wheelColPoint.m_vecPoint, wheelsSpinning, m_fContactSurfaceBrightness);
            return true;
        }
        return false;
    };
    ...
    if (CWeather::WetRoads > 0.4f && !CCullZones::CamNoRain()) {
        if (g_surfaceInfos.CreatesWheelSpray(wheelColPoint.m_nPieceTypeB)) {
            g_fx.AddWheelSpray(this, wheelColPoint.m_vecPoint, wheelsSpinning, false, m_fContactSurfaceBrightness);
            return false;
        }
    }
    return true;
...
// 0x6D30E0
bool CVehicle::SetVehicleUpgradeFlags(int32 modelId, int32 upgradeId, int32& replacing) {
    ...
    case 15: {
        ...
        const auto GetNitroValue = [&]() -> int8 {
            if (modelId == ModelIndices::MI_NITRO_BOTTLE_LARGE) {
                return 5;
            } else if (modelId == ModelIndices::MI_NITRO_BOTTLE_DOUBLE) {
                return 10;
            }
            return 2;
        };
        ...
        if (handlingFlags.bNosInst) {
            replacing = ModelIndices::MI_NITRO_BOTTLE_SMALL;
        }
...
// 0x6D3210
bool CVehicle::ClearVehicleUpgradeFlags(int32 modelId, int32 upgradeId) {
...
// 0x6D3700
RpAtomic* CVehicle::CreateReplacementAtomic(CBaseModelInfo* mi, RwFrame* parentFrame, int32 flags, bool isDamaged, bool bIsWheel) {
    // flags flags combined from `eAtomicComponentFlag`
    ...
    SetVehicleAtomicVisibility(atomic, flags & ~(ATOMIC_IS_DAM_STATE | ATOMIC_IS_OK_STATE));
...
// 0x6D3830
void CVehicle::AddReplacementUpgrade(int32 modelId, int32 componentId) {
    // NOTE: 0x6D38D5 - 0x6D390B => `SetupUpgradeAtomicRendering`

    ((void(__thiscall*)(CVehicle*, int32, int32))0x6D3830)(this, modelId, componentId);
}

// 0x6D39E0
void CVehicle::RemoveReplacementUpgrade(int32 componentId) {
    auto frameOfUpgrade = CClumpModelInfo::GetFrameFromId(m_pRwClump, componentId);
...
// 0x6D3A50
int32 CVehicle::GetReplacementUpgrade(int32 componentId) {
    auto frame = CClumpModelInfo::GetFrameFromId(m_pRwClump, componentId);

    struct {
        int32     nodeId;
        RpAtomic* atomic;
    } data = { componentId, nullptr };
...
// 0x6D3B30
void CVehicle::SetHasslePosId(int32 posId, bool used) {
    if (used) {
        m_nHasslePosId |= 1 << posId;
    } else {
        m_nHasslePosId ^= 1 << posId;
    }
}
...
// 0x6D3D30
void CVehicle::SetRopeHeightForHeli(float ropeHeight) const {
...
// 0x6D4290
CVector CVehicle::GetPlaneGunsPosition(int32 num) {
...
uint32 CVehicle::GetPlaneOrdnanceRateOfFire(eOrdnanceType type) {
    ...
    case MODEL_SEASPAR:
    case MODEL_RUSTLER:
        return (uint32)((float)1'000 / mult);
    case MODEL_HYDRA:
        return (uint32)((float)(type != 1 ? 1'000 : 500) / mult); // todo: heat trap / missile?
    default:
...
void CVehicle::FirePlaneGuns() {
    ...
    CWeapon weapon{ giWeaponType, 5'000 };
...
void CVehicle::FireUnguidedMissile(eOrdnanceType type, bool bCheckTime) {
    ...
    CWeapon weapon{ WEAPON_RLAUNCHER, 5'000 };
...
// 0x6D5490
void CVehicle::ReactToVehicleDamage(CPed* inflictor) {
    const auto DoReact = [=](CPed* pedThatLooks, CEntity* pedToLookAt) {
        g_ikChainMan.LookAt("ReactToVhclDam", pedThatLooks, pedToLookAt, CGeneral::GetRandomNumberInRange(2'000, 5'000), BONE_HEAD, nullptr, false, 0.25f, 500, 3, false);
    };
    ...
    if (m_pDriver) {
        if (passenger) {
            DoReact(m_pDriver, CGeneral::DoCoinFlip() ? inflictor : passenger);
        } else {
            DoReact(m_pDriver, inflictor);
        }
    }

    if (passenger) {
        DoReact(passenger, CGeneral::DoCoinFlip() ? inflictor : m_pDriver);
    }
}
...
// 0x6D5CF0
bool CVehicle::CanPedLeanOut(CPed* ped) const {
...
// 0x6D6C00
void CVehicle::ProcessWheel(CVector& forward, CVector& right, CVector& speed, CVector& offset, int32 noOfContactWheels, float driveAcceleration, float driveDeceleration, float adhesiveLimit, int8 wheelId, float* wheelPitchIncrement, tWheelState* wheelState, uint16 wheelDamage) {
    ...
    float rightLocal = 0.0f;
    float fwd = 0.0f;
    float contactSpeedFwd = DotProduct(forward, speed);
    float contactSpeedRight = DotProduct(right, speed);

    bBraking = driveDeceleration != 0.0f;
    bDriving = !bBraking;
    if (bDriving && driveAcceleration == 0.0f) {
        bDriving = false;
    }

    adhesiveLimit *= CTimer::GetTimeStep();
    if (*wheelState != WHEEL_STATE_NORMAL) {
        ...
        if (*wheelState == WHEEL_STATE_SPINNING) {
            if (GetStatus() == STATUS_PLAYER || GetStatus() == STATUS_REMOTE_CONTROLLED) {
                adhesiveLimit *= (1.0f - fabs(m_GasPedal) * WS_ALREADY_SPINNING_LOSS);
            }
        }
    }
    ...
    *wheelState = WHEEL_STATE_NORMAL;
    ...
    if (contactSpeedRight != 0.0f) {
        rightLocal = -(contactSpeedRight / noOfContactWheels);
        if (wheelDamage == WHEEL_STATUS_BURST) {
            float fwdspeed = std::min(contactSpeedFwd, fBurstSpeedMax);
            rightLocal += fwdspeed * CGeneral::GetRandomNumberInRange(-fBurstTyreMod, fBurstTyreMod);
        }
    }

    if (bDriving) {
        fwd = driveAcceleration;
        rightLocal = std::clamp(rightLocal, -adhesiveLimit, adhesiveLimit);
    } else if (contactSpeedFwd != 0.0f) {
        fwd = -contactSpeedFwd / noOfContactWheels;
        if (!bBraking && std::fabs(m_GasPedal) < 0.01f) {
            if (IsBike()) {
                driveDeceleration = gHandlingDataMgr.fWheelFriction * 0.6f / (m_pHandlingData->m_fMass + 200.0f);
            } else if (IsSubPlane()) {
                driveDeceleration = 0.0f;
            } else {
                driveDeceleration = gHandlingDataMgr.fWheelFriction / m_pHandlingData->m_fMass;

                if (driveDeceleration > 500.0f) {
                    driveDeceleration *= 0.1f;
                } else if (m_nModelIndex == MODEL_RCBANDIT) {
                    driveDeceleration *= 0.2f;
                }
            }
        }
        if (driveDeceleration > adhesiveLimit) {
            if (std::fabs(contactSpeedFwd) > 0.005f) {
                *wheelState = WHEEL_STATE_FIXED;
            }
        } else {
            fwd = std::clamp(fwd, -driveDeceleration, driveDeceleration);
        }
    }

    float speedSq = rightLocal * rightLocal + fwd * fwd;
    if (speedSq > adhesiveLimit * adhesiveLimit) {
        if (*wheelState != WHEEL_STATE_FIXED) {
            ...
            if (bDriving && tractionLimit * adhesiveLimit < std::fabs(fwd)) {
        ...
        fwd *= adhesiveLimit * tractionLoss / l;
        rightLocal *= adhesiveLimit * tractionLoss / l;
    }

    if (fwd != 0.0f || rightLocal != 0.0f) {
        bool separateTurnForce = false;
        CVector totalSpeed = fwd * forward + rightLocal * right;
        CVector turnDirection = totalSpeed;

        if (m_pHandlingData->m_fSuspensionAntiDiveMultiplier > 0.0f) {
            if (bBraking) {
                separateTurnForce = true;
                turnDirection -= (m_pHandlingData->m_fSuspensionAntiDiveMultiplier * forward * fwd);
            } else if (bDriving) {
                separateTurnForce = true;
                turnDirection -= (0.5f * m_pHandlingData->m_fSuspensionAntiDiveMultiplier * forward * fwd);
            }
        }
        ...
        float turnForce = turnSpeed * GetMass(offset, turnDirection);
        ApplyMoveForce(force * direction);
        ApplyTurnForce(turnForce * turnDirection, offset);
    }
}
...
// inline
inline void CVehicle::ReduceVehicleDamage(float& damage) {
    if (GetStatus() == STATUS_PLAYER) {
        // Only Mobile
        /*
        auto isStrap3 = strcmp((char*)CTheScripts::pActiveScripts->m_szName, "strap3");
        if (CTheScripts::pActiveScripts && !isStrap3) {
            damage /= 4.0f;
        }
        */
        // Only Mobile
...
// In Mobile
// It has special camera logic and does not use standard height settings
bool CVehicle::CanUseCameraHeightSetting() {
    if (!GetVehicleType()) {
        return false;
    }

    if (vehicleFlags.bIsLawEnforcer || vehicleFlags.bIsAmbulanceOnDuty
        || vehicleFlags.bIsFireTruckOnDuty) {
        return false;
    }

    short modelIndex = GetModelIndex();

    // Government/Emergency vehicles
    if (modelIndex == MODEL_FIRETRUK
        || modelIndex == MODEL_FIRELA
        || modelIndex == MODEL_FBIRANCH
        || modelIndex == MODEL_SWATVAN
        || modelIndex == MODEL_RHINO) {
        return false;
    }

    // Sports cars
    if (modelIndex == MODEL_CHEETAH) {
        return false;
    }

    // Off-road/Recreational
    if (modelIndex == MODEL_BFINJECT) {
        return false;
    }

    // Airplanes
    if (modelIndex == MODEL_CROPDUST) {
        return false;
    }

    // Special utility vehicles
    if (modelIndex == MODEL_TUG) {
        return false;
    }
...
// 0x6D7C90
void CVehicle::InflictDamage(CEntity* damager, eWeaponType weaponUsed, float damage, CVector pos) {
    ((void(__thiscall*)(CVehicle*, CEntity*, eWeaponType, float, CVector))0x6D7C90)(this, damager, weaponUsed, damage, pos);
}
...
bool CVehicle::BladeColSectorList(PtrListType& ptrList, CColModel& colModel, CMatrix& matrix, int16 rotorType, float damageMult) {
    ...
    for (auto* entity : ptrList) {
        ...
        const auto numColls = CCollision::ProcessColModels(
            matrix, colModel, entity->GetMatrix(), *entityCM, CWorld::m_aTempColPts, nullptr, nullptr, false
        );
        if (numColls <= 0) {
            continue;
        }
        ...
        if (std::fabs(m_GasPedal) > 0.05F || fMoveSpeed > 0.01F) {
            ...
            if (vecWorldThrustPos.z - 0.5F >= fWaterLevel) {
                ...
                if (!bPostCollision && bIsSlowingDown) {
                    ...
                    if (m_GasPedal < 0.0f && fTractionLoss > 0.0f || m_GasPedal > 0.0f && fTractionLoss < 0.0f) {
                        fTractionLoss *= -1.0F;
                    }
...
void CVehicle::AddExhaustParticles() {
    ...
    if (physicalFlags.bTouchingWater
        && CWaterLevel::GetWaterLevel(firstExhaustPos, pLevel, true)
    ...
    if (bHasDoubleExhaust) {
        ...
        if (physicalFlags.bTouchingWater
            && CWaterLevel::GetWaterLevel(secondExhaustPos, pLevel, true)
...
// always return false?
// 0x6DE880
bool CVehicle::AddSingleWheelParticles(tWheelState wheelState, uint32 wheelStatus, float susRatio, float speed, CColPoint* colPoint, CVector* pos, float outsideVec, int32 wheelIndex, uint32 skidmarkType, bool* bloody, uint32 optionFlags) {
    return ((bool(__thiscall*)(CVehicle*, tWheelState, uint32, float, float, CColPoint*, CVector*, float, int32, uint32, bool*, uint32))0x6DE880)(this, wheelState, wheelStatus, susRatio, speed, colPoint, pos, outsideVec, wheelIndex, skidmarkType, bloody, optionFlags);
}
...
// 0x6DF930
void CVehicle::RemoveVehicleUpgrade(int32 modelId) {
    ((void(__thiscall*)(CVehicle*, int32))0x6DF930)(this, modelId);
}

// 0x6DFA20
void CVehicle::AddUpgrade(int32 modelId, int32 upgradeId) {
    ((void(__thiscall*)(CVehicle*, int32, int32))0x6DFA20)(this, modelId, upgradeId);
}

// 0x6DFC50
void CVehicle::UpdateTrailerLink(bool applyFullVelocityAtHookUp, bool applyDistToSpeed) {
    ((void(__thiscall*)(CVehicle*, bool, bool))0x6DFC50)(this, applyFullVelocityAtHookUp, applyDistToSpeed);
}

// 0x6E0050
void CVehicle::UpdateTractorLink(bool applyFullVelocityAtHookUp, bool applyDistToSpeed) {
    ((void(__thiscall*)(CVehicle*, bool, bool))0x6E0050)(this, applyFullVelocityAtHookUp, applyDistToSpeed);
}
...
// 0x6E0950
void CVehicle::ProcessSirenAndHorn(bool hornAvailable) {
    ((void(__thiscall*)(CVehicle*, bool))0x6E0950)(this, hornAvailable);
}

// 0x6E0A50
bool CVehicle::DoHeadLightEffect(eVehicleDummy lightId, CMatrix& matVehicle, bool isRight, bool forcedOff) {
    return ((bool(__thiscall*)(CVehicle*, int32, CMatrix&, bool, bool))0x6E0A50)(this, lightId, matVehicle, isRight, forcedOff);
}

// 0x6E0E20
void CVehicle::DoHeadLightBeam(eVehicleDummy lightId, CMatrix& matVehicle, bool isRight) {
    const auto* mi = GetVehicleModelInfo();
    CVector pointModelSpace = mi->GetModelDummyPosition(static_cast<eVehicleDummy>(2 * lightId));
    if (lightId == DUMMY_LIGHT_REAR_MAIN && pointModelSpace.IsZero()) {
        return;
    }

    CVector point = matVehicle.GetPosition() + matVehicle.TransformVector(pointModelSpace);
    if (!isRight) {
        point -= 2 * pointModelSpace.x * matVehicle.GetRight();
    }
    const CVector pointToCamDir = Normalized(TheCamera.GetPosition() - point);
    const auto    alpha = (uint8)((1.0f - std::fabs(DotProduct(pointToCamDir, matVehicle.GetForward()))) * 32.0f);
    ...
    const CVector lightNormal = Normalized(matVehicle.GetForward() - matVehicle.GetUp() * angleMult);
    const CVector lightRight = Normalized(CrossProduct(lightNormal, pointToCamDir));
    const CVector lightPos = point - matVehicle.GetForward() * 0.1f;
...
// 0x6E1440
void CVehicle::DoHeadLightReflectionSingle(CMatrix& matVehicle, bool isRight) {
    auto vehOffset = GetDummyPositionObjSpace(DUMMY_LIGHT_FRONT_MAIN);
    if (!isRight) {
        vehOffset.x *= -1.f;
    }
    const auto lightFwd2D = CVector2D(matVehicle.GetForward()).Normalized();
    const auto lightRight2D = CVector2D(matVehicle.GetRight()).Normalized();
    ...
    CShadows::StoreCarLightShadow(
        ...
        GetPosition() + CVector(lightRight2D.x * vehOffset.x + lightFwd2D.x * offsetDistance, lightRight2D.y * vehOffset.x + lightFwd2D.y * offsetDistance, 2.0f),
        shdwFront.x,
        shdwFront.y,
        shdwSide.x,
        shdwSide.y,
...
// 0x6E1720
void CVehicle::DoHeadLightReflection(CMatrix& matVehicle, eVehicleLightsFlags lightFlags, bool left, bool right) {
    if (lightFlags & VEHICLE_LIGHTS_TWIN) {
        if (left && right) {
            DoHeadLightReflectionTwin(matVehicle);
        } else if (left || right) {
            DoHeadLightReflectionSingle(matVehicle, right);
        }
    } else {
        if (m_nModelIndex == MODEL_COMBINE) {
            DoHeadLightReflectionTwin(matVehicle);
        } else if (right) {
            DoHeadLightReflectionSingle(matVehicle, true);
        }
    }
}

// 0x6E1780
bool CVehicle::DoTailLightEffect(int32 lightId, CMatrix& matVehicle, bool isRight, bool forcedOff, eVehicleLightsFlags lightFlags, bool lightsOn) {
    return ((bool(__thiscall*)(CVehicle*, int32, CMatrix&, bool, bool, eVehicleLightsFlags, bool))0x6E1780)(this, lightId, matVehicle, isRight, forcedOff, lightFlags, lightsOn);
}

// unknown, search
void CVehicle::DoReverseLightEffect(int32 lightId, CMatrix& matVehicle, bool isRight, bool forcedOff, eVehicleLightsFlags lightFlags, bool lightsOn) {
    return;
}

// 0x6E1A60
void CVehicle::DoVehicleLights(CMatrix& matVehicle, eVehicleLightsFlags lightFlags) {
    ((void(__thiscall*)(CVehicle*, CMatrix&, uint32))0x6E1A60)(this, matVehicle, lightFlags);
}
...
void CVehicle::DoFixedMachineGuns() {
    ...
    } else if (CTimer::GetTimeInMS() > m_nGunFiringTime + 1'400) {
        m_nAmmoInClip = 20;
    }
}
...
// In Mobile
/*
CVector CVehicle::GetGasTankPosition() {
    CVector gasTank;
    // Code...
}
*/
...
// In Mobile
/*
void CVehicle::SetTappedGasTankVehicle(CEntity* tappedGasTankVehicle) {
    if (m_pTappedGasTankVehicle) {
        m_pTappedGasTankVehicle->CleanUpOldReference(&m_pTappedGasTankVehicle);
    }
    m_pTappedGasTankVehicle = tappedGasTankVehicle;
    if (tappedGasTankVehicle) {
        tappedGasTankVehicle->RegisterReference(&m_pTappedGasTankVehicle);
    }
}
*/

// In Mobile
bool CVehicle::GetHasDualExhausts() {
    return m_pHandlingData->m_bDoubleExhaust;
}
